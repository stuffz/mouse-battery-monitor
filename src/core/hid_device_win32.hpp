...
public:
    ...
    static std::vector<DeviceInfo> EnumerateDevices(USHORT vid, USHORT pid)
    {
        ...
        for (DWORD memberIndex = 0; SetupDiEnumDeviceInterfaces(deviceInfoSet, nullptr, &hidGuid,
...
private:
    ...
    static std::optional<DeviceInfo> GetDeviceInfo(HDEVINFO deviceInfoSet,
                                                   ...
                                                   USHORT targetVid, USHORT targetPid)
    {
        DWORD requiredSize = 0;
        SetupDiGetDeviceInterfaceDetailW(deviceInfoSet, &interfaceData, nullptr, 0, &requiredSize,
                                         nullptr);
        ...
        if (!SetupDiGetDeviceInterfaceDetailW(deviceInfoSet, &interfaceData, detailPtr,
                                              requiredSize, nullptr, nullptr))
        {
            return std::nullopt;
        }

        HANDLE h = CreateFileW(detailPtr->DevicePath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
        ...
        std::optional<DeviceInfo> result =
            ExtractDeviceInfo(h, detailPtr->DevicePath, targetVid, targetPid);
        CloseHandle(h);
        return result;
    }

    static std::optional<DeviceInfo> ExtractDeviceInfo(HANDLE h, const wchar_t *path,
                                                       USHORT targetVid, USHORT targetPid)
    {
        HIDD_ATTRIBUTES attrib{};
        attrib.Size = sizeof(HIDD_ATTRIBUTES);
        if (!HidD_GetAttributes(h, &attrib) || attrib.VendorID != targetVid ||
            attrib.ProductID != targetPid)
        {
            return std::nullopt;
        }
